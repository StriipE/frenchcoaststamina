...
router.get('/high', function (req,res){
    ...
    con.query('SELECT user.PlayerID, user.Name, user.CountryCode, country.Name AS CountryName, country.Flag AS CountryFlag, COUNT(*) AS Played, ROUND(SUM(score_high_histo.FCSPoints),2) AS Score from scores_high ' +
              ...
              'INNER JOIN country ON user.CountryCode = country.Code ' +
...
router.get('/low', function (req,res){
    ...
    con.query('SELECT user.PlayerID, user.Name, user.CountryCode, country.Name AS CountryName, country.Flag AS CountryFlag, COUNT(*) AS Played, SUM(Fantastics) AS Fantastics, SUM(Excellents) AS Excellents, ' +
